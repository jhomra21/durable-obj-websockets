...
const Slider = <T extends ValidComponent = "div">(
  ...
  return (
    <SliderPrimitive.Root
      class={cn("relative flex w-full touch-none select-none flex-col items-center cursor-default", local.class)}
      {...others}
    />
  )
}
...
const SliderTrack = <T extends ValidComponent = "div">(
  ...
  return (
    <SliderPrimitive.Track
      class={cn("relative h-2 w-full grow rounded-full bg-secondary cursor-pointer", local.class)}
      {...others}
    />
  )
}
...
const SliderThumb = <T extends ValidComponent = "span">(
  ...
  return (
    <SliderPrimitive.Thumb
      class={cn(
        "top-[-6px] block size-5 rounded-full border-2 border-primary bg-background ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 cursor-grab active:cursor-grabbing",
        local.class
      )}
      {...others}
    >
